...
if [ -z "${REMOTE_SOURCES_DIR:-}" ]; then
  ...
  WORKING_DIR="$(pwd)"
  SKUPPER_DIR="$WORKING_DIR/router/app"
  ...
  tar -zxf skupper-router.tar.gz --one-top-level="${SKUPPER_DIR}" --strip-components 1
else
...
sudo cmake --install "${LWS_BUILD_DIR}"

# Read about DESTDIR here - https://www.gnu.org/prep/standards/html_node/DESTDIR.html
sudo DESTDIR="${LWS_INSTALL_DIR}" cmake --install "${LWS_BUILD_DIR}"
...
if [ -z "$PLATFORM" ]; then
    ...
    if [ "$PLATFORM" = "amd64" ]; then
        ...
        ./configure
        sudo make install
        sudo DESTDIR="${LIBUNWIND_INSTALL_DIR}" make install
        popd
        #endregion libunwind
    else
...
sudo python3 -m pip install --disable-pip-version-check --ignore-installed --prefix="$PROTON_INSTALL_DIR/usr" "$(find "$PROTON_BUILD_DIR/python/dist" -name 'python_qpid_proton*.whl')" --break-system-packages
